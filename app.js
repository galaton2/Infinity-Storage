...
/* Память теперь складывает заметки, документы, фото, видео и аудио. */
...
/* Telegram-текстовый файл: пробуем загрузить его содержимое и сохраняем кэш. */
...
async function saveSmartNote() {
  ...
  if (!text) return toast("Напишите мысль");
...
function saveJournal() {
  ...
  const text = input.value.trim();
  if (!text) return toast("Введите запись");
...
function renderFiles() {
  ...
  if (!items.length) {
    gallery.innerHTML = '<div class="empty-files">Пусто</div>';
    return;
  }
    ...
    if (item.type === "photo" && safeUrl(item.url)) {
      ...
      card.innerHTML = `
        <div class="text-card-body">
          ...
          <br><br>
          ${esc(clean(item.text).slice(0, 220) || "Текст не был передан сервером")}
        </div>
      `;
...
/* Чат: берёт только максимум 5 релевантных записей, чтобы не тратить токены. */
...
async function sendChatMessage() {
  ...
  if (!notes.length) return chatMessage("Не нашёл подходящих заметок.", "ai");
...
function openBrainMap() {
  ...
  if (!db.notes.length || typeof window.ForceGraph3D !== "function") {
    mount.innerHTML = '<p class="brain-empty-message">Добавьте несколько заметок с тегами.</p>';
    return;
  }
  ...
  /* Управление для телефона: один палец — вращение, два — масштаб. */
...
function openDetails(item) {
  ...
  overlay.innerHTML = `
    <section class="note-details-card" role="dialog" aria-modal="true">
      ...
      <div class="note-details-date">${esc(new Date(item.date || Date.now()).toLocaleString("ru-RU"))}</div>
      <h2>${esc(item.title || "Файл")}</h2>
      <div class="note-details-text">${esc(item.text || "Сервер не передал содержимое файла.").replaceAll("\n", "<br>")}</div>
    </section>
  `;
...
function openJournalCalendar() {
  ...
  for (let day = 1; day <= count; day += 1) {
    ...
    days += `<span class="calendar-day ${saved.has(key) ? "has-entry" : "no-entry"}">${day}</span>`;
  }
  ...
  overlay.innerHTML = `
    <section class="calendar-card">
      <button class="calendar-close" type="button" aria-label="Закрыть">
        <svg class="svg-icon" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
      </button>
  ...
  overlay.onclick = (event) => {
    if (event.target === overlay || event.target.closest(".calendar-close")) overlay.remove();
  };
